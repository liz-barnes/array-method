...
$('body').on('click', '#new-york-btn', e => {
  renderNewYorkBusinesses();
})
...
const renderManufacturingBusinesses = () => {
  outEl.innerHTML = "";
...
$('body').on('click', '#manufacturing-btn', e => {
  renderManufacturingBusinesses();
})
...
const totalRainfall = monthlyRainfall.reduce(
  (rainTotal, monthlyRain) => rainTotal += monthlyRain, 0
)
...
const sentence = words.reduce(
  (theSentence, nextWord)  => theSentence += ` ${nextWord}`, ""
)
